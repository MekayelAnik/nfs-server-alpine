...
boot_helper_get_version_flags() {
  ...
  local flags=('--nfs-version' "$requested_version" '--no-nfs-version' 3)
  ...
  if [[ "$requested_version" = '4' ]]; then
    flags+=('--no-nfs-version' 4.1)
  fi
  if [[ "$requested_version" = '4.1' ]]; then
    flags+=('--no-nfs-version' 4.2)
  fi
